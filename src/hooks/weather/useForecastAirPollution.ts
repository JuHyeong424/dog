...
// useForecastAirPollution 훅이 인자로 받을 객체의 타입을 정의하는 인터페이스입니다.
...
/**
 * 위도와 경도를 기반으로 해당 위치의 대기 오염 '예보' 정보를 가져오는 React Query 커스텀 훅입니다.
 * @param {useForecastAirPollutionProps} { lon, lat } - 위도와 경도
...
export function useForecastAirPollution({ lon, lat }: useForecastAirPollutionProps) {
  ...
  return useQuery({
    ...
    queryFn: async() => {
      // 우리 서버의 Next.js API 라우트(/api/forecastAirPollution)로 GET 요청을 보냅니다.
      // 위도와 경도는 URL 쿼리 파라미터로 전달됩니다.
      const res = await axios.get(`/api/forecastAirPollution?lat=${lat}&lon=${lon}`);
      // API 응답에서 실제 대기 오염 예보 데이터를 반환합니다.
      return res.data;
    },
