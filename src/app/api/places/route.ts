...
export async function GET(request: Request) {
  ...
  try {
    // 1. Google Places API (Nearby Search)를 호출하여 주변 장소를 검색합니다.
    ...
    // 2. Distance Matrix API를 호출하기 위해 목적지 좌표 목록을 생성합니다.
    const destinations = places.map((p: any) => `${p.geometry.location.lat},${p.geometry.location.lng}`).join('|');

    // Distance Matrix API를 호출하여 각 장소까지의 거리와 시간을 계산합니다.
    ...
    // 3. 장소 정보와 거리/시간 정보를 결합하여 최종 결과를 생성합니다.
    const combinedResults = places.map((place: any, index: number) => {
      ...
      return {
        ...
        distance: distanceInfo?.distance?.text || 'N/A', // 거리
        duration: distanceInfo?.duration?.text || 'N/A', // 소요 시간
        geometry: place.geometry, // 지도에 표시하기 위한 좌표 정보
      };
    });

    // 결합된 결과를 클라이언트에 JSON 형태로 반환합니다.
